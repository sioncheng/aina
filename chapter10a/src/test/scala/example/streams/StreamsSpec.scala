...
class StreamsSpec
  ...
  "a producer of fixed elements 1,2,3 and a function" should {
    "allow a consumer to receive only even numbers" in {
      ...
      val producer: Source[Int, NotUsed] = Source(elements)
      val processor: Flow[Int, Int, NotUsed] = Flow[Int].filter(_ % 2 == 0)
      val consumer: Sink[Int, Future[Seq[Int]]] = Sink.seq

      val future = producer.via(processor).toMat(consumer)(Keep.right).run()
  ...
  "a producer of fixed elements 1,2,3 and a function" should {
    "allow when consumed see the side effects | simple test version" in {
      ...
      def storeDB(value: Int): Unit = fakeDB = fakeDB :+ value
  ...
  "a producer of fixed elements 1,2,3 and a function" should {
    "allow when consumed see the side effects | simple test version 2" in {
      var fakeDB: List[Int] = List()
      def storeDB(value: Int): Unit = fakeDB = fakeDB :+ value
      ...
      val future = producer.via(processor).runForeach(i => storeDB(i))
  ...
  "an infinite producer with a consumer creating side effect" should {
    "be cancellable" in {
      val liveSource: Source[String, Cancellable] =
        Source.tick(0.second, 1.seconds, s"Hello, World")

      val masking = Flow[String].map(_.replaceAll("World", "xyz"))

      def dbFakeInsert(value: String): Unit = println(s"inserting: $value")
