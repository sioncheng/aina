    ...
    with LogCapturing {
        ...
        "Among two actors, NO parent/child related, the watcher" must {
            "be able to be notified with Terminated when watched actor stops" in {
                ...
                var logprocessor = spawn(Behaviors.receiveMessagePartial[String] {
                    case "stop" =>
                        Behaviors.stopped
                })

                watcher ! SimplifiedFileWatcher.Watch(logprocessor.ref)
        ...
        "Among two actors, parent/child related, the watcher" must {
            ...
            "getting notified by Termination if it a child that only stopped" in {
                val probe = TestProbe[String]
                val watcher = spawn(ParentWatcher(probe.ref))
